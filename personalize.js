...
(function () {
  ...
  function full() { return state.fullName.trim() || (LANG() === "ar" ? "الاسم الكامل" : "Full Name"); }
  ...
  var SENDER_AR = { father: "من والدك", mother: "من والدتك", brother: "من أخيك", sister: "من أختك", teacherM: "من معلمك", teacherF: "من معلمتك", other: "من" };
  var SENDER_EN = { father: "From Your Father", mother: "From Your Mother", brother: "From Your Brother", sister: "From Your Sister", teacherM: "From Your Teacher", teacherF: "From Your Teacher", other: "From" };
  // The message page is generated ONLY when every required field is filled.
  function messageComplete() {
    var m = state.msg;
    return !!(m.type && m.name.trim() && m.date && m.text.trim());
  }
  ...
  function drawOverlay(ctx, o, W, H) {
    if (o.kind === "cover") {
      ...
      drawStack(ctx, [{ text: dateLine(), sizeR: 0.028, weight: 700 }],
  ...
  async function generate() {
    ...
    try {
      ...
      setStatus("", "");
      showDone();              // success message
      downloadBtn.disabled = false; // no lock — repeat generation allowed
      redirectHome();          // back to homepage after 3s
    } catch (err) {
      console.error(err);
  ...
  var LABELS = {
    ar: {
      ...
      full: { boy: "الاسم الكامل للطفل", girl: "الاسم الكامل للطفلة" },
      fph: { boy: "مثال: عبدالرحمن", girl: "مثال: سلمى" },
      uph: { boy: "مثال: عبدالرحمن محمد أحمد", girl: "مثال: سلمى محمد أحمد" },
    },
    en: {
      first: { boy: "First Name (Boy)", girl: "First Name (Girl)" },
      full: { boy: "Full Name (Boy)", girl: "Full Name (Girl)" },
      fph: { boy: "Example: Abdulrahman", girl: "Example: Salma" },
      uph: { boy: "Example: Abdulrahman Mohammed Ahmad", girl: "Example: Salma Mohammed Ahmad" },
    },
  };
  function updateLabels() {
    var l = LANG() === "ar" ? "ar" : "en", g = isBoy() ? "boy" : "girl";
  ...
  window.addEventListener("momin:lang", function () { updateLabels(); updateMockup(); });
